...
export class ProfileComponent {
    ...
    this.contentfulService.getPersonalInfo().subscribe({
      next: (entry) => {
        ...
        if (entry) {
          ...
          this.name = info.name;
          this.job = info.role;
          this.profileImage = info.profileImage.fields.file.url;
          this.email.email = info.email;
          this.phone.phone = info.phoneNumber;
          this.linkedin.url = info.linkedinLink;
          this.linkedin.username = info.linkedinUsername;
          this.github.url = info.githubLink;
          this.github.username = info.githubUsername;
          this.birthday.birthday = new Date(info.birthday);
        }
      },
  ...
  formatBirthdayDate(arg0: Date): string {
    const options: Intl.DateTimeFormatOptions = { year: 'numeric', month: 'short', day: '2-digit' };
    return arg0.toLocaleDateString('en-US', options);
  }
}