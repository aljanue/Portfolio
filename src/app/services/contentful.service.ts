...
export class ContentfulService {
  ...
  getProjects() {
    if (!this.projectsCache$) {
      this.projectsCache$ = this.getDataWithCache(
        'cache_projects', 
  ...
  getExperience() {
    if (!this.experienceCache$) {
      this.experienceCache$ = this.getDataWithCache(
        'cache_experience', 
  ...
  getEducation() {
    if (!this.educationCache$) {
      this.educationCache$ = this.getDataWithCache(
        'cache_education', 
  ...
  getSkills() {
    if (!this.skillsCache$) {
      this.skillsCache$ = this.getDataWithCache(
        'cache_skills', 
        () => this.getEntries('skill')
      );
    }
  ...
  getHonours() {
    if (!this.honoursCache$) {
      this.honoursCache$ = this.getDataWithCache(
        'cache_honours', 
        () => this.getEntries('honours')
      );
    }
  ...
  getCertifications() {
    if (!this.certificationsCache$) {
      this.certificationsCache$ = this.getDataWithCache(
  ...
  getDoingCards() {
    if (!this.doingCardsCache$) {
      this.doingCardsCache$ = this.getDataWithCache(
        'cache_doing_cards', 
  ...
  getPersonalInfo() {
    if (!this.personalInfoCache$) {
      this.personalInfoCache$ = this.getDataWithCache(
        'cache_personal_info', 
